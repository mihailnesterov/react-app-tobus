...
function StationsFilterGroup(props) {
    ...
    return (
        <ListGroup flush 
            ...
            { props.items.length > 0 ?
                ...
                <ListGroupItem 
                    key={index} 
                    className={index === +props.items.length-1 ? "px-2 py-1 text-primary border-bottom-0" : "px-2 py-1 text-primary" }
                    style={{"backgroundColor":"transparent"}} 
                    tag="a" 
                    href="#" 
                    onClick={props.filterStations}
                    >
                        { filter }
                </ListGroupItem>) : false
            }
...
export default StationsFilterGroup;
